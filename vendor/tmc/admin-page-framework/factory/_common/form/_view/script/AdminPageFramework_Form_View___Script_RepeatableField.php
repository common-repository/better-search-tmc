...
class TMC_v1_0_6_AdminPageFramework_Form_View___Script_RepeatableField extends TMC_v1_0_6_AdminPageFramework_Form_View___Script_Base {
...
(function ( $ ) {
     ...
     */
    $.fn.updateTMC_v1_0_6_AdminPageFrameworkRepeatableFields = function( aSettings ) {
        ...
        var _oRepeatableButtons = $( nodeThis ).find( '.tmcapf-repeatable-field-buttons' )
            .filter( function() {                                       
                return $( this ).closest( '.tmcapf-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
            });       
...
// @todo event.preventDefault();        
            ...
            $( this ).addTMC_v1_0_6_AdminPageFrameworkRepeatableField();
...
// @todo event.preventDefault();        
            $( this ).removeTMC_v1_0_6_AdminPageFrameworkRepeatableField();
            return false; // will not click after that
        });

        /* If the number of fields is less than the set minimum value, add fields. */
        var _sFieldID           = _oRepeatableAddButtons.first().closest( '.tmcapf-field' ).attr( 'id' );
        var _nCurrentFieldCount = $( '#' + _sFieldsContainerID ).find( '.tmcapf-field' )
            .filter( function() {                 
                return $( this ).closest( '.tmcapf-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
            })        
            .length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentFieldCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentFieldCount ) > 0 ) {     
                $( '#' + _sFieldID ).addTMC_v1_0_6_AdminPageFrameworkRepeatableField( _sFieldID );  
            }
        }
     ...
     */
    $.fn.addTMC_v1_0_6_AdminPageFrameworkRepeatableField = function( sFieldContainerID ) {
        
        if ( 'undefined' === typeof sFieldContainerID ) {
            var sFieldContainerID = $( this ).closest( '.tmcapf-field' ).attr( 'id' );
        }

        var nodeFieldContainer  = $( '#' + sFieldContainerID );
        var nodeNewField        = nodeFieldContainer.clone(); // clone without bind events.
        var nodeFieldsContainer = nodeFieldContainer.closest( '.tmcapf-fields' );
        ...
        if ( ! _aOptions ) {     
            var _nodeButtonContainer = nodeFieldContainer.find( '.tmcapf-repeatable-field-buttons' )
                .filter( function() {                 
                    return $( this ).closest( '.tmcapf-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                })
                .first();
        ...
        var _oInnerFields       = nodeFieldsContainer.find( '.tmcapf-field' ) 
                                    .filter( function() {                 
                                        return $( this ).closest( '.tmcapf-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                                    });
        var _oRepeatableButtons = nodeFieldContainer.find( '.tmcapf-repeatable-field-buttons' )
                                    .filter( function() {                 
                                        return $( this ).closest( '.tmcapf-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                                    }); 
         ...
         */
        nodeNewField.updateTMC_v1_0_6_AdminPageFrameworkRepeatableFields();
        ...
        $( nodeNewField ).find( '.tmcapf-field' ).addBack().trigger( 
            'tmcapf_repeated_field', 
            [ 
                0, // call type, 0 : repeatable fields, 1: repeatable sections
                jQuery( nodeNewField ).closest( '.tmcapf-fields' )    // model container
            ]
        );
        ...
        var nodeRemoveButtons = nodeFieldsContainer
            ...
            .filter( function() {                      
                return $( this ).closest( '.tmcapf-fields' ).attr( 'id' ) === _sFieldsContainerID;                
            })
        ...
        nodeFieldsContainer.children( '.tmcapf-field' ).children( '.delimiter' ).show().last().hide();
        ...
        var _incrementFieldAttributes = function( oElement, oFieldsContainer ) {
            ...
            oElement.find( '.tmcapf-field,.tmcapf-fields,.tmcapf-fieldset' ).incrementAttributes(
     ...
     * Removes a repeatable field.
      This method is called when the user presses the - repeatable button.
     */
    $.fn.removeTMC_v1_0_6_AdminPageFrameworkRepeatableField = function() {
        
        /* Need to remove the element: the field container */
        var nodeFieldContainer  = $( this ).closest( '.tmcapf-field' );
        var nodeFieldsContainer = $( this ).closest( '.tmcapf-fields' );
        var _sFieldsContainerID = nodeFieldsContainer.attr( 'id' );
        var _aOptions = nodeFieldsContainer.data( 'repeatable' );
