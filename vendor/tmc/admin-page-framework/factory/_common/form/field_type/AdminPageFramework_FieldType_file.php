...
class TMC_v1_0_6_AdminPageFramework_FieldType_text extends TMC_v1_0_6_AdminPageFramework_FieldType {
    public $aFieldTypeSlugs = array('text', 'password', 'date', 'datetime', 'datetime-local', 'email', 'month', 'search', 'tel', 'url', 'week',);
    protected $aDefaultKeys = array();
    protected function getStyles() {
        return ".tmcapf-field.tmcapf-field-text > .tmcapf-input-label-container {vertical-align: middle; }.tmcapf-field.tmcapf-field-text > .tmcapf-input-label-container.tmcapf-field-text-multiple-labels {display: block;}";
    }
    ...
    private function _getFieldOutputByLabel($sKey, $sLabel, $aField) {
        ...
        $_sClassSelector = $_bIsArray ? 'tmcapf-field-text-multiple-labels' : '';
        ...
        $_aOutput = array($this->getElementByLabel($aField['before_label'], $sKey, $aField['label']), "<div class='tmcapf-input-label-container {$_sClassSelector}'>", "<label for='" . $_aInputAttributes['id'] . "'>", $this->getElementByLabel($aField['before_input'], $sKey, $aField['label']), $_sLabel ? "<span " . $this->getLabelContainerAttributes($aField, 'tmcapf-input-label-string') . ">" . $_sLabel . "</span>" : '', "<input " . $this->getAttributes($_aInputAttributes) . " />", $this->getElementByLabel($aField['after_input'], $sKey, $aField['label']), "</label>", "</div>", $this->getElementByLabel($aField['after_label'], $sKey, $aField['label']),);
        return implode('', $_aOutput);
    }
    }
    class TMC_v1_0_6_AdminPageFramework_FieldType_file extends TMC_v1_0_6_AdminPageFramework_FieldType_text {
